...
- The verifier judge is configurable — it is not tied to Gemini. Set
  ...
  (defaults to `https://openrouter.ai/api/v1`), `deepseek/...` (hosted
  DeepSeek), or `openai/...` with a `base_url` pointing at a local
  vLLM/SGLang endpoint (full fine-grained logprob reward needs a server that
